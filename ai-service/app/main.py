...
from app.routers import analyze, line_bot, compliance, aeo, rag
from app.db.vector_db import init_db


@asynccontextmanager
async def lifespan(app: FastAPI):
    # 서버 시작 시 pgvector 확장 + 테이블 자동 생성
    if settings.vector_db_url:
        try:
            init_db()
            print("✓ Vector DB 초기화 완료")
        except Exception as e:
            print(f"⚠ Vector DB 초기화 실패 (RAG 비활성화): {e}")
    yield
...
@app.get("/health")
async def health():
    return {"status": "ok", "service": "mediflow-ai", "rag": bool(settings.vector_db_url)}
